...
public class UserDAO {
	static ArrayList<UserAccount> listU = new ArrayList<UserAccount>();
	...
	public static UserAccount selectUSER(String userId) {
		...
		try {
			...
			ps = conn.prepareStatement(
					"select user_id,username, email_verification_hash, email_verification_attempts, status, created_time, "
					+ "firstname, lastname, gender, date_of_birth, age, score, code, payment, avatar, role_id from user where user_id=?");
	...
	public static ArrayList<UserAccount> getUser(String userId) throws SQLException, ClassNotFoundException {
	...
	public static String insertRow(UserAccount user) {
		...
		try {
			...
			ps1 = conn.prepareStatement(
					"insert into user (username, email, password, create_time, email_verification_hash, status, role_id) values (?,?,?,?,?,?,?)");
	...
	public static UserAccount verifyLogin(String inputEmail, String inputPassword) {
		...
		try {
			conn = MySQLConnUtils.getMySQLConnection();
			ps = conn.prepareStatement(
					"select user_id, username, email, status, create_time, role_id from user where email =? and password =?");
	...
	public static UserAccount selectUSERbyEmail(String inputEmail) {
		...
		try {
			conn = MySQLConnUtils.getMySQLConnection();
			ps = conn.prepareStatement(
					"select user_id, username, email, email_verification_hash, email_verification_attempts, status, create_time from user where email = ?");
			ps.setString(1, inputEmail);
			res = ps.executeQuery();
			if (res != null) {
				while (res.next()) {
	...
	public void updateInformation(String userId, String firstname, String lastname, String birthday, String gender) throws ParseException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = MySQLConnUtils.getMySQLConnection();
			ps = conn.prepareStatement("update user set firstname = ? , lastname = ? , date_of_birth = ? , gender = ?  where user_id = ?");
	...
	public static void main(String[] args) throws ParseException {
		...
		String dt = "20/10/2016";
		ud.updateInformation("1", "bbbbbb", "bbbbb", dt, "nam");
//		user.setUSERNAME("a");
